#include <cstdint>
#include "ppc_runtime.h"
#include "abi_bridge.h"
#include "memory.h"
#include "recomp_mod_loader.h"

extern "C" void func_807F88C0(CpuContext* MKW_RESTRICT ctx)
{
    uint32_t r0 = ctx->gpr[0];
    uint32_t r3 = ctx->gpr[3];
    uint32_t r4 = ctx->gpr[4];
    uint32_t r5 = ctx->gpr[5];

    goto loc_807F88C0;

loc_807F88C0:
{
    r4 = MemoryInline::FlatRead32((r3 + 100));
    r0 = 2;
    r5 = MemoryInline::FlatRead32((r3 + 428));
    r4 = MemoryInline::FlatRead32((r4 + 12));
    r4 = MemoryInline::FlatRead32((r4 + 32));
    r4 = (r4 + 20);
    MemoryInline::FlatWrite32((r3 + 408), r0);
    r0 = (r4 - r5);
    MemoryInline::FlatWrite32((r3 + 432), r0);
    ctx->gpr[0] = r0;
    ctx->gpr[4] = r4;
    ctx->gpr[5] = r5;
    return;
}

}

// RECOMP_GUEST_ABI gpr_read=0x00000008 gpr_write=0x00000031 gpr_return=0x00000010 fpr_read=0x00000000 fpr_write=0x00000000 fpr_return=0x00000000 cr_read=0x00 cr_write=0x00 xer_read=0 xer_write=0 fence=0
// RECOMP_REGISTRATION base 0x807F88C0 func_807F88C0 preserves=true fpr_mask=0x00000000
