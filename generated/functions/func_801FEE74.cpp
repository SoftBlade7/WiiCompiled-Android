#include <cstdint>
#include "ppc_runtime.h"
#include "abi_bridge.h"
#include "memory.h"
#include "recomp_mod_loader.h"

extern "C" void func_801FEE74(CpuContext* MKW_RESTRICT ctx)
{
    goto loc_801FEE74;

loc_801FEE74:
{
    InvokeDirectCpu<0x80205B6Cu>(ctx);
    return;
}

}

// RECOMP_GUEST_ABI gpr_read=0xC0002FFB gpr_write=0xC0000FFB gpr_return=0x00000018 fpr_read=0x00000000 fpr_write=0x00000000 fpr_return=0x00000000 cr_read=0x01 cr_write=0x01 xer_read=1 xer_write=0 fence=0
// RECOMP_REGISTRATION base 0x801FEE74 func_801FEE74 preserves=true fpr_mask=0x00000000
