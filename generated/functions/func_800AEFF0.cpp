#include <cstdint>
#include "ppc_runtime.h"
#include "abi_bridge.h"
#include "memory.h"
#include "recomp_mod_loader.h"

extern "C" void func_800AEFF0(CpuContext* MKW_RESTRICT ctx)
{
    uint32_t cr0_0 = 0;
    uint32_t r4_addr_0 = 0;
    uint32_t r4_addr_1 = 0;
    uint32_t r4_addr_2 = 0;
    uint32_t r4_addr_3 = 0;
    uint32_t r5_addr_0 = 0;
    uint32_t r6_addr_0 = 0;

    uint32_t r0 = ctx->gpr[0];
    uint32_t r3 = ctx->gpr[3];
    uint32_t r4 = ctx->gpr[4];
    uint32_t r5 = ctx->gpr[5];
    uint32_t r6 = ctx->gpr[6];
    uint32_t r7 = ctx->gpr[7];
    uint32_t cr = ctx->cr;
    uint32_t xer = ctx->xer;

    goto loc_800AEFF0;

loc_800AEFF0:
{
}

loc_800AEFF4:
{
    if ((static_cast<int32_t>(r4) != static_cast<int32_t>(0))) {
        goto loc_800AF068;
    }
}

loc_800AEFF8:
{
    r0 = MemoryInline::FlatRead32(r3);
    SetCRResident(cr, xer, 0, static_cast<int32_t>(r0), static_cast<int32_t>(0));
}

loc_800AF000:
{
    if (((cr & 0x20000000u) == 0)) {
        goto loc_800AF030;
    }
}

loc_800AF004:
{
    r4 = MemoryInline::FlatRead16((r3 + 10));
    r0 = 0;
    r4 = (r5 + r4);
    MemoryInline::FlatWrite32((r4 + 4), r0);
    MemoryInline::FlatWrite32(r4, r0);
    r4 = MemoryInline::FlatRead16((r3 + 8));
    MemoryInline::FlatWrite32(r3, r5);
    r0 = (r4 + 1);
    MemoryInline::FlatWrite32((r3 + 4), r5);
    MemoryInline::FlatWrite16((r3 + 8), static_cast<uint16_t>(r0));
    ctx->gpr[0] = r0;
    ctx->gpr[4] = r4;
    ctx->gpr[6] = r6;
    ctx->gpr[7] = r7;
    ctx->cr = cr;
    return;
}

loc_800AF030:
{
    r6 = MemoryInline::FlatRead16((r3 + 10));
    r0 = 0;
    r4 = MemoryInline::FlatRead32((r3 + 4));
    r6_addr_0 = (r6 + r5);
    MemoryInline::FlatWrite32(r6_addr_0, r4);
    r6 = r6_addr_0;
    MemoryInline::FlatWrite32((r6 + 4), r0);
    r4 = MemoryInline::FlatRead32((r3 + 4));
    r0 = MemoryInline::FlatRead16((r3 + 10));
    r4 = (r4 + r0);
    MemoryInline::FlatWrite32((r4 + 4), r5);
    r4 = MemoryInline::FlatRead16((r3 + 8));
    MemoryInline::FlatWrite32((r3 + 4), r5);
    r0 = (r4 + 1);
    MemoryInline::FlatWrite16((r3 + 8), static_cast<uint16_t>(r0));
    ctx->gpr[0] = r0;
    ctx->gpr[4] = r4;
    ctx->gpr[6] = r6;
    ctx->gpr[7] = r7;
    ctx->cr = cr;
    return;
}

loc_800AF068:
{
    r0 = MemoryInline::FlatRead32(r3);
    SetCRResident(cr, xer, 0, static_cast<uint32_t>(r4), static_cast<uint32_t>(r0));
}

loc_800AF070:
{
    if (((cr & 0x20000000u) == 0)) {
        goto loc_800AF0DC;
    }
}

loc_800AF074:
{
    SetCRResident(cr, xer, 0, static_cast<int32_t>(r0), static_cast<int32_t>(0));
}

loc_800AF078:
{
    if (((cr & 0x20000000u) == 0)) {
        goto loc_800AF0A8;
    }
}

loc_800AF07C:
{
    r4 = MemoryInline::FlatRead16((r3 + 10));
    r0 = 0;
    r4 = (r5 + r4);
    MemoryInline::FlatWrite32((r4 + 4), r0);
    MemoryInline::FlatWrite32(r4, r0);
    r4 = MemoryInline::FlatRead16((r3 + 8));
    MemoryInline::FlatWrite32(r3, r5);
    r0 = (r4 + 1);
    MemoryInline::FlatWrite32((r3 + 4), r5);
    MemoryInline::FlatWrite16((r3 + 8), static_cast<uint16_t>(r0));
    ctx->gpr[0] = r0;
    ctx->gpr[4] = r4;
    ctx->gpr[6] = r6;
    ctx->gpr[7] = r7;
    ctx->cr = cr;
    return;
}

loc_800AF0A8:
{
    r4 = MemoryInline::FlatRead16((r3 + 10));
    r0 = 0;
    r4_addr_0 = (r4 + r5);
    MemoryInline::FlatWrite32(r4_addr_0, r0);
    r4 = r4_addr_0;
    r0 = MemoryInline::FlatRead32(r3);
    MemoryInline::FlatWrite32((r4 + 4), r0);
    r4 = MemoryInline::FlatRead32(r3);
    r0 = MemoryInline::FlatRead16((r3 + 10));
    r4_addr_1 = (r4 + r0);
    MemoryInline::FlatWrite32(r4_addr_1, r5);
    r4 = MemoryInline::FlatRead16((r3 + 8));
    MemoryInline::FlatWrite32(r3, r5);
    r0 = (r4 + 1);
    MemoryInline::FlatWrite16((r3 + 8), static_cast<uint16_t>(r0));
    ctx->gpr[0] = r0;
    ctx->gpr[4] = r4;
    ctx->gpr[6] = r6;
    ctx->gpr[7] = r7;
    ctx->cr = cr;
    return;
}

loc_800AF0DC:
{
    r0 = MemoryInline::FlatRead16((r3 + 10));
    r4_addr_2 = (r4 + r0);
    r6 = MemoryInline::FlatRead32(r4_addr_2);
    r7 = (r5 + r0);
    r5_addr_0 = (r5 + r0);
    MemoryInline::FlatWrite32(r5_addr_0, r6);
    r6 = (r6 + r0);
    MemoryInline::FlatWrite32((r7 + 4), r4);
    MemoryInline::FlatWrite32((r6 + 4), r5);
    r0 = MemoryInline::FlatRead16((r3 + 10));
    r4_addr_3 = (r4 + r0);
    MemoryInline::FlatWrite32(r4_addr_3, r5);
    r4 = MemoryInline::FlatRead16((r3 + 8));
    r0 = (r4 + 1);
    MemoryInline::FlatWrite16((r3 + 8), static_cast<uint16_t>(r0));
    ctx->gpr[0] = r0;
    ctx->gpr[4] = r4;
    ctx->gpr[6] = r6;
    ctx->gpr[7] = r7;
    ctx->cr = cr;
    return;
}

}

// RECOMP_GUEST_ABI gpr_read=0x00000039 gpr_write=0x000000D1 gpr_return=0x00000010 fpr_read=0x00000000 fpr_write=0x00000000 fpr_return=0x00000000 cr_read=0x01 cr_write=0x01 xer_read=1 xer_write=0 fence=0
// RECOMP_REGISTRATION base 0x800AEFF0 func_800AEFF0 preserves=true fpr_mask=0x00000000
