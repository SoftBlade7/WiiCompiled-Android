#include <cstdint>
#include "ppc_runtime.h"
#include "abi_bridge.h"
#include "memory.h"
#include "recomp_mod_loader.h"

extern "C" void func_8016FF40(CpuContext* MKW_RESTRICT ctx)
{
    uint32_t cr0_0 = 0;
    uint32_t r0_rot_0 = 0;
    uint32_t r0_rot_1 = 0;
    uint32_t r0_rot_2 = 0;
    uint32_t r0_rot_3 = 0;
    uint32_t r3_addr_0 = 0;
    uint32_t r3_addr_1 = 0;
    uint32_t r3_addr_2 = 0;
    uint32_t r3_addr_3 = 0;

    uint32_t r0 = ctx->gpr[0];
    uint32_t r1 = ctx->gpr[1];
    uint32_t r2 = ctx->gpr[2];
    uint32_t r3 = ctx->gpr[3];
    uint32_t r4 = ctx->gpr[4];
    uint32_t r30 = ctx->gpr[30];
    uint32_t r31 = ctx->gpr[31];
    PPC_FPR f0 = ctx->fpr[0];
    PPC_FPR f1 = ctx->fpr[1];
    PPC_FPR f2 = ctx->fpr[2];
    PPC_FPR f3 = ctx->fpr[3];
    PPC_FPR f4 = ctx->fpr[4];
    PPC_FPR f5 = ctx->fpr[5];
    PPC_FPR f6 = ctx->fpr[6];
    uint32_t cr = ctx->cr;
    uint32_t ctr = ctx->ctr;
    uint32_t xer = ctx->xer;

    goto loc_8016FF40;

loc_8016FF40:
{
    MemoryInline::FlatWriteRam32((r1 + -16), r1);
    r1 = (r1 + -16);
    r0 = ctx->lr;
    f0.d = MemoryInline::FlatReadFloat32((r2 + -26784));
    MemoryInline::FlatWriteRam32((r1 + 20), r0);
    SetCRFloatResident(cr, 0, f1.d, f0.d);
}

loc_8016FF54:
{
    MemoryInline::FlatWriteRam32((r1 + 12), r31);
    r31 = r4;
    MemoryInline::FlatWriteRam32((r1 + 8), r30);
    r30 = r3;
    cr = PpcCrLogicalResident(cr, static_cast<uint32_t>(7), static_cast<uint32_t>(2), static_cast<uint32_t>(0), static_cast<uint32_t>(2));
    if (((cr & 0x20000000u) != 0)) {
        goto loc_8016FF78;
    }
}

loc_8016FF6C:
{
    f0.d = MemoryInline::FlatReadFloat32((r2 + -26780));
    SetCRFloatResident(cr, 0, f1.d, f0.d);
}

loc_8016FF74:
{
    if (((cr & 0x40000000u) == 0)) {
        goto loc_8016FF7C;
    }
}

loc_8016FF78:
{
    r31 = 0;
}

loc_8016FF7C:
{
    f2.d = MemoryInline::FlatReadFloat32((r2 + -26776));
    f0.d = MemoryInline::FlatReadFloat32((r2 + -26772));
    f1.d = PpcFmulsInline(f2.d, f1.d);
    f1.d = static_cast<double>(PpcForceSingleValueInline(f1.d / f0.d));
    ctx->lr = 0x8016FF90u;
    ctx->gpr[0] = r0;
    ctx->gpr[1] = r1;
    ctx->gpr[3] = r3;
    ctx->gpr[30] = r30;
    ctx->gpr[31] = r31;
    ctx->fpr[0] = f0;
    ctx->fpr[1] = f1;
    ctx->fpr[2] = f2;
    ctx->fpr[3] = f3;
    ctx->fpr[4] = f4;
    ctx->fpr[5] = f5;
    ctx->fpr[6] = f6;
    ctx->cr = cr;
    ctx->ctr = ctr;
    InvokeDirectCpu<0x8001B590u>(ctx);
    r0 = ctx->gpr[0];
    r1 = ctx->gpr[1];
    r3 = ctx->gpr[3];
    r4 = ctx->gpr[4];
    r30 = ctx->gpr[30];
    r31 = ctx->gpr[31];
    f0 = ctx->fpr[0];
    f1 = ctx->fpr[1];
    f2 = ctx->fpr[2];
    f3 = ctx->fpr[3];
    f4 = ctx->fpr[4];
    f5 = ctx->fpr[5];
    f6 = ctx->fpr[6];
    cr = ctx->cr;
    ctr = ctx->ctr;
    xer = ctx->xer;
    SetCRResident(cr, xer, 0, static_cast<uint32_t>(r31), static_cast<uint32_t>(6));
}

loc_8016FF94:
{
    f5.d = static_cast<double>(PpcForceSingleValueInline(f1.d));
    if (((cr & 0x40000000u) != 0)) {
        goto loc_80170098;
    }
}

loc_8016FF9C:
{
    r3 = 0x80290000u;
    r0_rot_1 = PpcRotl32Inline(static_cast<uint32_t>(r31), static_cast<uint32_t>(2));
    r0 = (r0_rot_1 & -4);
    r3 = (r3 + -23688);
    r3_addr_1 = (r3 + r0);
    r3 = MemoryInline::FlatRead32(r3_addr_1);
    ctr = r3;
    switch (static_cast<uint32_t>(ctr)) {
    case 0x80170098u:
        goto loc_80170098;
        break;
    case 0x8016FFB4u:
        goto loc_8016FFB4;
        break;
    case 0x8016FFC8u:
        goto loc_8016FFC8;
        break;
    case 0x8016FFE4u:
        goto loc_8016FFE4;
        break;
    case 0x80170000u:
        goto loc_80170000;
        break;
    case 0x8017002Cu:
        goto loc_8017002C;
        break;
    case 0x8017005Cu:
        goto loc_8017005C;
        break;
    default:
        ctx->gpr[0] = r0;
        ctx->gpr[1] = r1;
        ctx->gpr[3] = r3;
        ctx->gpr[30] = r30;
        ctx->gpr[31] = r31;
        ctx->fpr[0] = f0;
        ctx->fpr[1] = f1;
        ctx->fpr[2] = f2;
        ctx->fpr[3] = f3;
        ctx->fpr[4] = f4;
        ctx->fpr[5] = f5;
        ctx->fpr[6] = f6;
        ctx->cr = cr;
        ctx->ctr = ctr;
        InvokeIndirectJump(ctr, ctx);
        return;
    }
}

loc_8016FFB4:
{
    f0.d = MemoryInline::FlatReadFloat32((r2 + -26768));
    f1.d = MemoryInline::FlatReadFloat32((r2 + -26764));
    f3.d = PpcFmulsInline(f0.d, f5.d);
    f6.d = MemoryInline::FlatReadFloat32((r2 + -26784));
    goto loc_801700A4;
}

loc_8016FFC8:
{
    f2.d = MemoryInline::FlatReadFloat32((r2 + -26760));
    f0.d = (-(f5.d));
    f6.d = MemoryInline::FlatReadFloat32((r2 + -26784));
    f1.d = static_cast<double>(PpcForceSingleValueInline(f2.d - f5.d));
    f1.d = static_cast<double>(PpcForceSingleValueInline(f2.d / f1.d));
    f3.d = PpcFmulsInline(f0.d, f1.d);
    goto loc_801700A4;
}

loc_8016FFE4:
{
    f2.d = MemoryInline::FlatReadFloat32((r2 + -26760));
    f0.d = (-(f5.d));
    f3.d = MemoryInline::FlatReadFloat32((r2 + -26784));
    f1.d = static_cast<double>(PpcForceSingleValueInline(f2.d - f5.d));
    f6.d = static_cast<double>(PpcForceSingleValueInline(f2.d / f1.d));
    f1.d = PpcFmulsInline(f0.d, f6.d);
    goto loc_801700A4;
}

loc_80170000:
{
    f3.d = MemoryInline::FlatReadFloat32((r2 + -26760));
    f1.d = MemoryInline::FlatReadFloat32((r2 + -26756));
    f2.d = static_cast<double>(PpcForceSingleValueInline(f3.d - f5.d));
    f0.d = static_cast<double>(PpcForceSingleValueInline(f5.d - f1.d));
    f2.d = PpcFmulsInline(f2.d, f2.d);
    f0.d = PpcFmulsInline(f5.d, f0.d);
    f2.d = static_cast<double>(PpcForceSingleValueInline(f3.d / f2.d));
    f3.d = PpcFmulsInline(f2.d, f0.d);
    f1.d = PpcFmulsInline(f1.d, f2.d);
    f6.d = (-(f2.d));
    goto loc_801700A4;
}

loc_8017002C:
{
    f4.d = MemoryInline::FlatReadFloat32((r2 + -26760));
    f0.d = MemoryInline::FlatReadFloat32((r2 + -26748));
    f3.d = static_cast<double>(PpcForceSingleValueInline(f4.d - f5.d));
    f2.d = MemoryInline::FlatReadFloat32((r2 + -26752));
    f1.d = static_cast<double>(PpcForceSingleValueInline(f4.d + f5.d));
    f3.d = PpcFmulsInline(f3.d, f3.d);
    f0.d = PpcFmulsInline(f0.d, f1.d);
    f1.d = static_cast<double>(PpcForceSingleValueInline(f4.d / f3.d));
    f6.d = PpcFmulsInline(f2.d, f1.d);
    f1.d = PpcFmulsInline(f0.d, f1.d);
    f3.d = PpcFmulsInline(f6.d, f5.d);
    goto loc_801700A4;
}

loc_8017005C:
{
    f4.d = MemoryInline::FlatReadFloat32((r2 + -26760));
    f0.d = MemoryInline::FlatReadFloat32((r2 + -26756));
    f3.d = static_cast<double>(PpcForceSingleValueInline(f4.d - f5.d));
    f1.d = MemoryInline::FlatReadFloat32((r2 + -26748));
    f2.d = PpcFmulsInline(f0.d, f5.d);
    f0.d = MemoryInline::FlatReadFloat32((r2 + -26744));
    f1.d = PpcFmulsInline(f1.d, f5.d);
    f3.d = PpcFmulsInline(f3.d, f3.d);
    f2.d = PpcFmulsInline(f2.d, f5.d);
    f3.d = static_cast<double>(PpcForceSingleValueInline(f4.d / f3.d));
    f2.d = PpcFmulsInline(f3.d, f2.d);
    f1.d = PpcFmulsInline(f1.d, f3.d);
    f6.d = PpcFmulsInline(f0.d, f3.d);
    f3.d = static_cast<double>(PpcForceSingleValueInline(f4.d - f2.d));
    goto loc_801700A4;
}

loc_80170098:
{
    f1.d = MemoryInline::FlatReadFloat32((r2 + -26784));
    f3.d = MemoryInline::FlatReadFloat32((r2 + -26760));
    f6.d = f1.d;
}

loc_801700A4:
{
    MemoryInline::FlatWriteFloat32((r30 + 16), f3.d);
    MemoryInline::FlatWriteFloat32((r30 + 20), f1.d);
    MemoryInline::FlatWriteFloat32((r30 + 24), f6.d);
    r31 = MemoryInline::FlatRead32((r1 + 12));
    r30 = MemoryInline::FlatRead32((r1 + 8));
    r0 = MemoryInline::FlatRead32((r1 + 20));
    ctx->lr = r0;
    r1 = (r1 + 16);
    ctx->gpr[0] = r0;
    ctx->gpr[1] = r1;
    ctx->gpr[3] = r3;
    ctx->gpr[30] = r30;
    ctx->gpr[31] = r31;
    ctx->fpr[0] = f0;
    ctx->fpr[1] = f1;
    ctx->fpr[2] = f2;
    ctx->fpr[3] = f3;
    ctx->fpr[4] = f4;
    ctx->fpr[5] = f5;
    ctx->fpr[6] = f6;
    ctx->cr = cr;
    ctx->ctr = ctr;
    return;
}

}

// RECOMP_GUEST_ABI gpr_read=0xFFFFDFFF gpr_write=0xFFFFDFFB gpr_return=0x00000018 fpr_read=0xFF8003FF fpr_write=0xFF8003FF fpr_return=0x00000002 cr_read=0xFF cr_write=0xFF xer_read=1 xer_write=1 fence=0
// RECOMP_REGISTRATION base 0x8016FF40 func_8016FF40 preserves=true fpr_mask=0x00000000
