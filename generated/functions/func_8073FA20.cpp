#include <cstdint>
#include "ppc_runtime.h"
#include "abi_bridge.h"
#include "memory.h"
#include "recomp_mod_loader.h"

extern "C" void func_8073FA20(CpuContext* MKW_RESTRICT ctx)
{
    uint32_t cr0_0 = 0;

    uint32_t r0 = ctx->gpr[0];
    uint32_t r3 = ctx->gpr[3];
    uint32_t r4 = ctx->gpr[4];
    uint32_t r5 = ctx->gpr[5];
    uint32_t r6 = ctx->gpr[6];
    uint32_t r7 = ctx->gpr[7];
    uint32_t cr = ctx->cr;
    uint32_t xer = ctx->xer;

    goto loc_8073FA20;

loc_8073FA20:
{
    r5 = r3;
    r6 = 0;
    goto loc_8073FA4C;
}

loc_8073FA2C:
{
    r7 = MemoryInline::FlatRead32((r5 + 12));
    r0 = MemoryInline::FlatRead32((r7 + 20));
    MemoryInline::FlatWrite32((r7 + 24), r0);
}

loc_8073FA3C:
{
    if ((static_cast<int32_t>(r0) >= static_cast<int32_t>(r4))) {
        goto loc_8073FA44;
    }
}

loc_8073FA40:
{
    MemoryInline::FlatWrite32((r7 + 24), r4);
}

loc_8073FA44:
{
    r5 = (r5 + 4);
    r6 = (r6 + 1);
}

loc_8073FA4C:
{
    r0 = MemoryInline::FlatRead32((r3 + 4));
    SetCRResident(cr, xer, 0, static_cast<int32_t>(r6), static_cast<int32_t>(r0));
}

loc_8073FA54:
{
    if (((cr & 0x80000000u) != 0)) {
        goto loc_8073FA2C;
    }
}

loc_8073FA58:
{
    ctx->gpr[0] = r0;
    ctx->gpr[5] = r5;
    ctx->gpr[6] = r6;
    ctx->gpr[7] = r7;
    ctx->cr = cr;
    return;
}

}

// RECOMP_GUEST_ABI gpr_read=0x000000F9 gpr_write=0x000000E1 gpr_return=0x00000000 fpr_read=0x00000000 fpr_write=0x00000000 fpr_return=0x00000000 cr_read=0x01 cr_write=0x01 xer_read=1 xer_write=0 fence=0
// RECOMP_REGISTRATION base 0x8073FA20 func_8073FA20 preserves=true fpr_mask=0x00000000
