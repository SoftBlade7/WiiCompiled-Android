#include <cstdint>
#include "ppc_runtime.h"
#include "abi_bridge.h"
#include "memory.h"
#include "recomp_mod_loader.h"

extern "C" void func_80239680(CpuContext* MKW_RESTRICT ctx)
{
    uint32_t r0 = ctx->gpr[0];
    uint32_t r3 = ctx->gpr[3];

    goto loc_80239680;

loc_80239680:
{
    r3 = (r3 + -72);
    // inline leaf 0x80238F3C (3 guest instruction(s))
    r0 = MemoryInline::FlatRead8((r3 + 338));
    r3 = (r0 & 1);
    // end of inlined leaf 0x80238F3C
    ctx->gpr[0] = r0;
    ctx->gpr[3] = r3;
    return;
}

}

// RECOMP_GUEST_ABI gpr_read=0x00000008 gpr_write=0x00000009 gpr_return=0x00000008 fpr_read=0x00000000 fpr_write=0x00000000 fpr_return=0x00000000 cr_read=0x00 cr_write=0x00 xer_read=0 xer_write=0 fence=0
// RECOMP_REGISTRATION base 0x80239680 func_80239680 preserves=true fpr_mask=0x00000000
