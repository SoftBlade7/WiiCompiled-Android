#include <cstdint>
#include "ppc_runtime.h"
#include "abi_bridge.h"
#include "memory.h"
#include "recomp_mod_loader.h"

extern "C" void func_8083B8D0(CpuContext* MKW_RESTRICT ctx)
{
    uint32_t cr0_0 = 0;

    uint32_t r0 = ctx->gpr[0];
    uint32_t r1 = ctx->gpr[1];
    uint32_t r3 = ctx->gpr[3];
    uint32_t r4 = ctx->gpr[4];
    uint32_t r5 = ctx->gpr[5];
    uint32_t r6 = ctx->gpr[6];
    uint32_t cr = ctx->cr;
    uint32_t xer = ctx->xer;

    goto loc_8083B8D0;

loc_8083B8D0:
{
    MemoryInline::FlatWriteRam32((r1 + -16), r1);
    r1 = (r1 + -16);
    r0 = ctx->lr;
    MemoryInline::FlatWriteRam32((r1 + 20), r0);
    r0 = MemoryInline::FlatRead32((r4 + 576));
    SetCRResident(cr, xer, 0, static_cast<int32_t>(r0), static_cast<int32_t>(-100));
}

loc_8083B8E4:
{
    if (((cr & 0x20000000u) != 0)) {
        goto loc_8083B904;
    }
}

loc_8083B8E8:
{
    SetCRResident(cr, xer, 0, static_cast<int32_t>(r0), static_cast<int32_t>(1));
}

loc_8083B8EC:
{
    if (((cr & 0x20000000u) != 0)) {
        goto loc_8083B90C;
    }
}

loc_8083B8F0:
{
    SetCRResident(cr, xer, 0, static_cast<int32_t>(r0), static_cast<int32_t>(0));
}

loc_8083B8F4:
{
    if (((cr & 0x20000000u) != 0)) {
        goto loc_8083B92C;
    }
}

loc_8083B8F8:
{
    SetCRResident(cr, xer, 0, static_cast<int32_t>(r0), static_cast<int32_t>(2));
}

loc_8083B8FC:
{
    if (((cr & 0x20000000u) != 0)) {
        goto loc_8083B94C;
    }
}

loc_8083B900:
{
    goto loc_8083B95C;
}

loc_8083B904:
{
    ctx->lr = 0x8083B908u;
    ctx->gpr[0] = r0;
    ctx->gpr[1] = r1;
    ctx->gpr[3] = r3;
    ctx->gpr[4] = r4;
    ctx->gpr[5] = r5;
    ctx->gpr[6] = r6;
    ctx->cr = cr;
    InvokeDirectCpu<0x80837B4Cu>(ctx);
    r0 = ctx->gpr[0];
    r1 = ctx->gpr[1];
    r3 = ctx->gpr[3];
    r4 = ctx->gpr[4];
    r5 = ctx->gpr[5];
    r6 = ctx->gpr[6];
    cr = ctx->cr;
    xer = ctx->xer;
    goto loc_8083B99C;
}

loc_8083B90C:
{
    r5 = 0x809C0000u;
    r0 = 1;
    r6 = MemoryInline::FlatRead32((r5 + -10456));
    r5 = r4;
    r4 = 107;
    MemoryInline::FlatWrite32((r6 + 5992), r0);
    ctx->lr = 0x8083B928u;
    ctx->gpr[0] = r0;
    ctx->gpr[1] = r1;
    ctx->gpr[3] = r3;
    ctx->gpr[4] = r4;
    ctx->gpr[5] = r5;
    ctx->gpr[6] = r6;
    ctx->cr = cr;
    InvokeDirectCpu<0x80837720u>(ctx);
    r0 = ctx->gpr[0];
    r1 = ctx->gpr[1];
    r3 = ctx->gpr[3];
    r4 = ctx->gpr[4];
    r5 = ctx->gpr[5];
    r6 = ctx->gpr[6];
    cr = ctx->cr;
    xer = ctx->xer;
    goto loc_8083B95C;
}

loc_8083B92C:
{
    r5 = 0x809C0000u;
    r0 = 0;
    r6 = MemoryInline::FlatRead32((r5 + -10456));
    r5 = r4;
    r4 = 107;
    MemoryInline::FlatWrite32((r6 + 5992), r0);
    ctx->lr = 0x8083B948u;
    ctx->gpr[0] = r0;
    ctx->gpr[1] = r1;
    ctx->gpr[3] = r3;
    ctx->gpr[4] = r4;
    ctx->gpr[5] = r5;
    ctx->gpr[6] = r6;
    ctx->cr = cr;
    InvokeDirectCpu<0x80837720u>(ctx);
    r0 = ctx->gpr[0];
    r1 = ctx->gpr[1];
    r3 = ctx->gpr[3];
    r4 = ctx->gpr[4];
    r5 = ctx->gpr[5];
    r6 = ctx->gpr[6];
    cr = ctx->cr;
    xer = ctx->xer;
    goto loc_8083B95C;
}

loc_8083B94C:
{
    r5 = r4;
    r4 = 119;
    ctx->lr = 0x8083B958u;
    ctx->gpr[0] = r0;
    ctx->gpr[1] = r1;
    ctx->gpr[3] = r3;
    ctx->gpr[4] = r4;
    ctx->gpr[5] = r5;
    ctx->gpr[6] = r6;
    ctx->cr = cr;
    InvokeDirectCpu<0x80837720u>(ctx);
    r0 = ctx->gpr[0];
    r1 = ctx->gpr[1];
    r3 = ctx->gpr[3];
    r4 = ctx->gpr[4];
    r5 = ctx->gpr[5];
    r6 = ctx->gpr[6];
    cr = ctx->cr;
    xer = ctx->xer;
    goto loc_8083B99C;
}

loc_8083B95C:
{
    r6 = 0x809C0000u;
    r5 = 0;
    r4 = MemoryInline::FlatRead32((r6 + -10456));
    r3 = 0x809C0000u;
    r0 = MemoryInline::FlatRead32((r4 + 6016));
    r0 = (r0 | 2);
    MemoryInline::FlatWrite32((r4 + 6016), r0);
    r4 = MemoryInline::FlatRead32((r6 + -10456));
    MemoryInline::FlatWrite32((r4 + 5980), r5);
    r4 = MemoryInline::FlatRead32((r6 + -10456));
    r0 = MemoryInline::FlatRead32((r4 + 6016));
    r0 = (r0 & -2);
    MemoryInline::FlatWrite32((r4 + 6016), r0);
    r3 = MemoryInline::FlatRead32((r3 + 7736));
    r3 = MemoryInline::FlatRead32((r3 + 152));
    MemoryInline::FlatWrite32((r3 + 96), r5);
}

loc_8083B99C:
{
    r0 = MemoryInline::FlatRead32((r1 + 20));
    ctx->lr = r0;
    r1 = (r1 + 16);
    ctx->gpr[0] = r0;
    ctx->gpr[1] = r1;
    ctx->gpr[3] = r3;
    ctx->gpr[4] = r4;
    ctx->gpr[5] = r5;
    ctx->gpr[6] = r6;
    ctx->cr = cr;
    return;
}

}

// RECOMP_GUEST_ABI gpr_read=0xFFFFFFFF gpr_write=0xFFFFFFFF gpr_return=0x00000018 fpr_read=0xFFFFFFFF fpr_write=0xFFFFFFFF fpr_return=0x00000002 cr_read=0xFF cr_write=0xFF xer_read=1 xer_write=1 fence=1
// RECOMP_REGISTRATION base 0x8083B8D0 func_8083B8D0 preserves=true fpr_mask=0x00000000
