#include <cstdint>
#include "ppc_runtime.h"
#include "abi_bridge.h"
#include "memory.h"
#include "recomp_mod_loader.h"

extern "C" void func_8023F230(CpuContext* MKW_RESTRICT ctx)
{
    uint32_t cr0_0 = 0;
    uint32_t r1_psq_tmp_0 = 0;
    uint32_t r3_psq_tmp_0 = 0;
    uint32_t r3_psq_tmp_1 = 0;
    uint32_t r3_psq_tmp_2 = 0;
    uint32_t r4_psq_tmp_0 = 0;
    uint32_t r4_psq_tmp_1 = 0;
    uint32_t r4_psq_tmp_2 = 0;
    uint32_t r5_psq_tmp_0 = 0;
    uint32_t r5_psq_tmp_1 = 0;
    uint32_t r5_psq_tmp_2 = 0;
    uint32_t r5_psq_tmp_3 = 0;
    uint8_t* guest_range_0 = nullptr;
    uint8_t* guest_range_1 = nullptr;
    uint8_t* guest_range_2 = nullptr;

    uint32_t r0 = ctx->gpr[0];
    uint32_t r1 = ctx->gpr[1];
    uint32_t r2 = ctx->gpr[2];
    uint32_t r3 = ctx->gpr[3];
    uint32_t r4 = ctx->gpr[4];
    uint32_t r5 = ctx->gpr[5];
    uint32_t r30 = ctx->gpr[30];
    uint32_t r31 = ctx->gpr[31];
    PPC_FPR f0 = ctx->fpr[0];
    PPC_FPR f1 = ctx->fpr[1];
    PPC_FPR f2 = ctx->fpr[2];
    PPC_FPR f3 = ctx->fpr[3];
    PPC_FPR f4 = ctx->fpr[4];
    PPC_FPR f5 = ctx->fpr[5];
    PPC_FPR f6 = ctx->fpr[6];
    PPC_FPR f7 = ctx->fpr[7];
    PPC_FPR f8 = ctx->fpr[8];
    PPC_FPR f9 = ctx->fpr[9];
    PPC_FPR f10 = ctx->fpr[10];
    PPC_FPR f31 = ctx->fpr[31];
    uint32_t cr = ctx->cr;

    [[maybe_unused]] uint32_t mkw_gqr0 = ctx->gqr[0];

    goto loc_8023F230;

loc_8023F230:
{
    MemoryInline::FlatWriteRam32((r1 + -96), r1);
    r1 = (r1 + -96);
    r0 = ctx->lr;
    MemoryInline::FlatWriteRam32((r1 + 100), r0);
    MemoryInline::FlatWriteRamFloat64((r1 + 80), f31.d);
    // psq_store w=0 quant=0 (using PPC_PsqSt)
    r1_psq_tmp_0 = (r1 + 88);
    PPC_PsqStStackInline<0u, 0u>(ctx, r1_psq_tmp_0, PPC_PsFromScalarInline(f31.d));
    guest_range_0 = MemoryInline::ResolveRangeHost((r2 + -26360), 0, 2024u, true, false);
    f0.d = MemoryInline::ReadResolvedFloat32(guest_range_0, 1996u, (r2 + -24364));
    MemoryInline::FlatWriteRam32((r1 + 76), r31);
    r31 = (r3 + 124);
    MemoryInline::FlatWriteRam32((r1 + 72), r30);
    r30 = r3;
    f1.d = MemoryInline::FlatReadFloat32((r3 + 100));
    SetCRFloatResident(cr, 0, f1.d, f0.d);
}

loc_8023F260:
{
    if (((cr & 0x40000000u) == 0)) {
        goto loc_8023F47C;
    }
}

loc_8023F264:
{
    r0 = MemoryInline::FlatRead16((r3 + 104));
    r0 = (r0 & 4);
}

loc_8023F26C:
{
    if ((static_cast<int32_t>(r0) != static_cast<int32_t>(0))) {
        goto loc_8023F278;
    }
}

loc_8023F270:
{
    f31.d = MemoryInline::ReadResolvedFloat32(guest_range_0, 1992u, (r2 + -24368));
    goto loc_8023F27C;
}

loc_8023F278:
{
    f31.d = MemoryInline::ReadResolvedFloat32(guest_range_0, 2008u, (r2 + -24352));
}

loc_8023F27C:
{
    r3 = (r3 + 112);
    // inline leaf 0x8019AC68 (17 guest instruction(s))
}

loc_inl0_0x8019AC68:
{
    // psq_load w=0 quant=0 (using PPC_PsqL)
    PpcSetPairedFprInline(f0, PPC_PsqLGqrInline<0u, 0u>(ctx, mkw_gqr0, r3));
    f4.d = MemoryInline::ReadResolvedFloat32(guest_range_0, 0u, (r2 + -26360));
    PpcSetPairedFprInline(f0, PPC_PsMulInline(f0.d, f0.d));
    f1.d = MemoryInline::FlatReadFloat32((r3 + 8));
    f2.d = static_cast<double>(PpcForceSingleValueInline(f4.d - f4.d));
    PpcSetPairedFprInline(f1, PPC_PsMaddInline(PPC_PsFromScalarInline(f1.d), PPC_PsFromScalarInline(f1.d), f0.d));
    PpcSetPairedFprInline(f1, PPC_PsSum0Inline(f1.d, f0.d, f0.d));
    SetCRFloatResident(cr, 0, PPC_PsToScalarInline(f1.d), f2.d);
}

loc_inl0_0x8019AC88:
{
    if (((cr & 0x20000000u) != 0)) {
        goto loc_inl0_return;
    }
}

loc_inl0_0x8019AC8C:
{
    f0.d = PPC_Frsqrte(PPC_PsToScalarInline(f1.d));
    f3.d = MemoryInline::ReadResolvedFloat32(guest_range_0, 4u, (r2 + -26356));
    f2.d = PpcFmulsInline(f0.d, f0.d);
    f0.d = PpcFmulsInline(f0.d, f4.d);
    f2.d = PPC_Fnmsubs(f2.d, PPC_PsToScalarInline(f1.d), f3.d);
    f0.d = PpcFmulsInline(f2.d, f0.d);
    f1.d = PpcFmulsInline(PPC_PsToScalarInline(f1.d), f0.d);
    goto loc_inl0_cont_8019AC68;
}

loc_inl0_return:
{
    f1.d = PPC_PsToScalarInline(f1.d);
}

loc_inl0_cont_8019AC68:
{
    // end of inlined leaf 0x8019AC68
    f10.d = MemoryInline::ReadResolvedFloat32(guest_range_0, 2000u, (r2 + -24360));
    guest_range_1 = MemoryInline::ResolveRangeHost(r30, 0, 124u, true, false);
    f5.d = MemoryInline::ReadResolvedFloat32(guest_range_1, 120u, (r30 + 120));
    f6.d = static_cast<double>(PpcForceSingleValueInline(f10.d / f1.d));
    {
        const auto resolved_pair = MemoryInline::ReadResolvedPair32(guest_range_1, 112u);
        if (resolved_pair.valid) {
            f1.d = PpcBitCastToFloatInline(resolved_pair.second);
            f4.d = PpcBitCastToFloatInline(resolved_pair.first);
        } else {
            f1.d = MemoryInline::ReadResolvedFloat32(guest_range_1, 116u, (r30 + 116));
            f4.d = MemoryInline::ReadResolvedFloat32(guest_range_1, 112u, (r30 + 112));
        }
    }
    {
        const auto resolved_pair = MemoryInline::ReadResolvedPair32(guest_range_0, 2008u);
        if (resolved_pair.valid) {
            f0.d = PpcBitCastToFloatInline(resolved_pair.second);
            f3.d = PpcBitCastToFloatInline(resolved_pair.first);
        } else {
            f0.d = MemoryInline::ReadResolvedFloat32(guest_range_0, 2012u, (r2 + -24348));
            f3.d = MemoryInline::ReadResolvedFloat32(guest_range_0, 2008u, (r2 + -24352));
        }
    }
    f2.d = MemoryInline::ReadResolvedFloat32(guest_range_0, 1996u, (r2 + -24364));
    f9.d = PpcFmulsInline(f1.d, f6.d);
    f1.d = MemoryInline::ReadResolvedFloat32(guest_range_0, 1992u, (r2 + -24368));
    f8.d = PpcFmulsInline(f4.d, f6.d);
    f5.d = PpcFmulsInline(f5.d, f6.d);
    MemoryInline::FlatWriteRamFloat32((r1 + 48), f9.d);
    f4.d = std::fabs(f9.d);
    MemoryInline::FlatWriteRamFloat32((r1 + 44), f8.d);
    MemoryInline::FlatWriteRamFloat32((r1 + 52), f5.d);
    SetCRFloatResident(cr, 0, f4.d, f0.d);
}

loc_8023F2C8:
{
    {
        const auto resolved_pair = MemoryInline::ReadResolvedPair32(guest_range_1, 96u);
        if (resolved_pair.valid) {
            f0.d = PpcBitCastToFloatInline(resolved_pair.second);
            f6.d = PpcBitCastToFloatInline(resolved_pair.first);
        } else {
            f0.d = MemoryInline::ReadResolvedFloat32(guest_range_1, 100u, (r30 + 100));
            f6.d = MemoryInline::ReadResolvedFloat32(guest_range_1, 96u, (r30 + 96));
        }
    }
    f3.d = PpcFmulsInline(f3.d, f0.d);
    {
        const auto resolved_pair = MemoryInline::ReadResolvedPair32(guest_range_1, 88u);
        if (resolved_pair.valid) {
            f4.d = PpcBitCastToFloatInline(resolved_pair.second);
            f0.d = PpcBitCastToFloatInline(resolved_pair.first);
        } else {
            f4.d = MemoryInline::ReadResolvedFloat32(guest_range_1, 92u, (r30 + 92));
            f0.d = MemoryInline::ReadResolvedFloat32(guest_range_1, 88u, (r30 + 88));
        }
    }
    f7.d = PpcFmulsInline(f5.d, f3.d);
    MemoryInline::FlatWriteRamFloat32((r1 + 20), f2.d);
    f5.d = PpcFmulsInline(f9.d, f3.d);
    f3.d = PpcFmulsInline(f8.d, f3.d);
    MemoryInline::FlatWriteRamFloat32((r1 + 24), f1.d);
    f6.d = static_cast<double>(PpcForceSingleValueInline(f6.d + f7.d));
    f4.d = static_cast<double>(PpcForceSingleValueInline(f4.d + f5.d));
    MemoryInline::FlatWriteRamFloat32((r1 + 28), f2.d);
    f0.d = static_cast<double>(PpcForceSingleValueInline(f0.d + f3.d));
    MemoryInline::FlatWriteRamFloat32((r1 + 36), f4.d);
    MemoryInline::FlatWriteRamFloat32((r1 + 32), f0.d);
    MemoryInline::FlatWriteRamFloat32((r1 + 40), f6.d);
    if (((cr & 0x40000000u) == 0)) {
        goto loc_8023F328;
    }
}

loc_8023F310:
{
    SetCRFloatResident(cr, 0, f9.d, f2.d);
}

loc_8023F314:
{
    MemoryInline::FlatWriteRamFloat32((r1 + 24), f2.d);
    if (((cr & 0x40000000u) == 0)) {
        goto loc_8023F320;
    }
}

loc_8023F31C:
{
    goto loc_8023F324;
}

loc_8023F320:
{
    f10.d = f1.d;
}

loc_8023F324:
{
    MemoryInline::FlatWriteRamFloat32((r1 + 28), f10.d);
}

loc_8023F328:
{
    r3 = (r1 + 20);
    r4 = (r1 + 44);
    r5 = (r1 + 8);
    // inline leaf 0x8019ACCC (15 guest instruction(s))
    // psq_load w=0 quant=0 (using PPC_PsqL)
    PpcSetPairedFprInline(f1, PPC_PsqLStackInline<0u, 0u>(ctx, r4));
    f2.d = MemoryInline::FlatReadFloat32((r3 + 8));
    // psq_load w=0 quant=0 (using PPC_PsqL)
    PpcSetPairedFprInline(f0, PPC_PsqLStackInline<0u, 0u>(ctx, r3));
    PpcSetPairedFprInline(f6, PPC_PsMerge10Inline(f1.d, f1.d));
    f3.d = MemoryInline::FlatReadFloat32((r4 + 8));
    PpcSetPairedFprInline(f4, PPC_PsMulInline(f1.d, PPC_PsFromScalarInline(f2.d)));
    PpcSetPairedFprInline(f7, PPC_PsMuls0Inline(f1.d, f0.d));
    PpcSetPairedFprInline(f5, PPC_PsMsubInline(f0.d, PPC_PsFromScalarInline(f3.d), f4.d));
    PpcSetPairedFprInline(f8, PPC_PsMsubInline(f0.d, f6.d, f7.d));
    PpcSetPairedFprInline(f9, PPC_PsMerge11Inline(f5.d, f5.d));
    PpcSetPairedFprInline(f10, PPC_PsMerge01Inline(f5.d, f8.d));
    // psq_store w=1 quant=0 (using PPC_PsqSt)
    PPC_PsqStStackInline<1u, 0u>(ctx, r5, f9.d);
    PpcSetPairedFprInline(f10, PPC_PsNegInline(f10.d));
    // psq_store w=0 quant=0 (using PPC_PsqSt)
    r5_psq_tmp_0 = (r5 + 4);
    PPC_PsqStStackInline<0u, 0u>(ctx, r5_psq_tmp_0, f10.d);
    // end of inlined leaf 0x8019ACCC
    r3 = (r1 + 8);
    r4 = r3;
    // inline leaf 0x8019AC24 (17 guest instruction(s))
    // psq_load w=0 quant=0 (using PPC_PsqL)
    PpcSetPairedFprInline(f2, PPC_PsqLStackInline<0u, 0u>(ctx, r3));
    // psq_load w=1 quant=0 (using PPC_PsqL)
    r3_psq_tmp_0 = (r3 + 8);
    PpcSetPairedFprInline(f3, PPC_PsqLStackInline<1u, 0u>(ctx, r3_psq_tmp_0));
    PpcSetPairedFprInline(f5, PPC_PsMulInline(f2.d, f2.d));
    {
        const auto resolved_pair = MemoryInline::ReadResolvedPair32(guest_range_0, 0u);
        if (resolved_pair.valid) {
            f0.d = PpcBitCastToFloatInline(resolved_pair.first);
            f1.d = PpcBitCastToFloatInline(resolved_pair.second);
        } else {
            f0.d = MemoryInline::ReadResolvedFloat32(guest_range_0, 0u, (r2 + -26360));
            f1.d = MemoryInline::ReadResolvedFloat32(guest_range_0, 4u, (r2 + -26356));
        }
    }
    PpcSetPairedFprInline(f4, PPC_PsMaddInline(f3.d, f3.d, f5.d));
    PpcSetPairedFprInline(f4, PPC_PsSum0Inline(f4.d, f5.d, f3.d));
    f5.d = PPC_Frsqrte(PPC_PsToScalarInline(f4.d));
    f6.d = PpcFmulsInline(f5.d, f5.d);
    f0.d = PpcFmulsInline(f5.d, f0.d);
    f6.d = PPC_Fnmsubs(f6.d, PPC_PsToScalarInline(f4.d), f1.d);
    f5.d = PpcFmulsInline(f6.d, f0.d);
    PpcSetPairedFprInline(f2, PPC_PsMuls0Inline(f2.d, PPC_PsFromScalarInline(f5.d)));
    PpcSetPairedFprInline(f3, PPC_PsMuls0Inline(f3.d, PPC_PsFromScalarInline(f5.d)));
    // psq_store w=0 quant=0 (using PPC_PsqSt)
    PPC_PsqStStackInline<0u, 0u>(ctx, r4, f2.d);
    // psq_store w=1 quant=0 (using PPC_PsqSt)
    r4_psq_tmp_0 = (r4 + 8);
    PPC_PsqStStackInline<1u, 0u>(ctx, r4_psq_tmp_0, f3.d);
    // end of inlined leaf 0x8019AC24
    r3 = (r1 + 44);
    r4 = (r1 + 8);
    r5 = (r1 + 20);
    // inline leaf 0x8019ACCC (15 guest instruction(s))
    // psq_load w=0 quant=0 (using PPC_PsqL)
    PpcSetPairedFprInline(f1, PPC_PsqLStackInline<0u, 0u>(ctx, r4));
    f2.d = MemoryInline::FlatReadFloat32((r3 + 8));
    // psq_load w=0 quant=0 (using PPC_PsqL)
    PpcSetPairedFprInline(f0, PPC_PsqLStackInline<0u, 0u>(ctx, r3));
    PpcSetPairedFprInline(f6, PPC_PsMerge10Inline(f1.d, f1.d));
    f3.d = MemoryInline::FlatReadFloat32((r4 + 8));
    PpcSetPairedFprInline(f4, PPC_PsMulInline(f1.d, PPC_PsFromScalarInline(f2.d)));
    PpcSetPairedFprInline(f7, PPC_PsMuls0Inline(f1.d, f0.d));
    PpcSetPairedFprInline(f5, PPC_PsMsubInline(f0.d, PPC_PsFromScalarInline(f3.d), f4.d));
    PpcSetPairedFprInline(f8, PPC_PsMsubInline(f0.d, f6.d, f7.d));
    PpcSetPairedFprInline(f9, PPC_PsMerge11Inline(f5.d, f5.d));
    PpcSetPairedFprInline(f10, PPC_PsMerge01Inline(f5.d, f8.d));
    // psq_store w=1 quant=0 (using PPC_PsqSt)
    PPC_PsqStStackInline<1u, 0u>(ctx, r5, f9.d);
    PpcSetPairedFprInline(f10, PPC_PsNegInline(f10.d));
    // psq_store w=0 quant=0 (using PPC_PsqSt)
    r5_psq_tmp_1 = (r5 + 4);
    PPC_PsqStStackInline<0u, 0u>(ctx, r5_psq_tmp_1, f10.d);
    // end of inlined leaf 0x8019ACCC
    r3 = MemoryInline::ReadResolved16(guest_range_1, 0u, r30);
    r0 = 1127219200;
    MemoryInline::FlatWriteRam32((r1 + 60), r3);
    f3.d = MemoryInline::ReadResolvedFloat64(guest_range_0, 2016u, (r2 + -24344));
    MemoryInline::FlatWriteRam32((r1 + 56), r0);
    f1.d = MemoryInline::ReadResolvedFloat32(guest_range_0, 2008u, (r2 + -24352));
    f2.d = MemoryInline::FlatReadFloat64((r1 + 56));
    f0.d = MemoryInline::ReadResolvedFloat32(guest_range_1, 100u, (r30 + 100));
    f2.d = static_cast<double>(PpcForceSingleValueInline(f2.d - f3.d));
    f3.d = MemoryInline::ReadResolvedFloat32(guest_range_0, 1992u, (r2 + -24368));
    f0.d = PpcFmulsInline(f1.d, f0.d);
    f4.d = MemoryInline::FlatReadFloat32((r1 + 8));
    f1.d = MemoryInline::FlatReadFloat32((r1 + 32));
    f5.d = PpcFmulsInline(f31.d, f2.d);
    f3.d = PpcFmulsInline(f0.d, f3.d);
    f0.d = MemoryInline::FlatReadFloat32((r1 + 36));
    f2.d = MemoryInline::FlatReadFloat32((r1 + 40));
    f3.d = static_cast<double>(PpcForceSingleValueInline(f5.d / f3.d));
    f4.d = PpcFmulsInline(f4.d, f3.d);
    guest_range_2 = MemoryInline::ResolveRangeHost(r31, 0, 48u, false, true);
    MemoryInline::WriteResolvedFloat32(guest_range_2, 0u, r31, f4.d);
    f4.d = MemoryInline::FlatReadFloat32((r1 + 12));
    f4.d = PpcFmulsInline(f4.d, f3.d);
    MemoryInline::WriteResolvedFloat32(guest_range_2, 4u, (r31 + 4), f4.d);
    f4.d = MemoryInline::FlatReadFloat32((r1 + 16));
    f4.d = PpcFmulsInline(f4.d, f3.d);
    MemoryInline::WriteResolvedFloat32(guest_range_2, 8u, (r31 + 8), f4.d);
    f5.d = MemoryInline::FlatReadFloat32((r1 + 8));
    f4.d = MemoryInline::FlatReadFloat32((r1 + 12));
    f6.d = MemoryInline::FlatReadFloat32((r1 + 16));
    f5.d = PpcFmulsInline(f1.d, f5.d);
    f4.d = PpcFmulsInline(f0.d, f4.d);
    f6.d = PpcFmulsInline(f2.d, f6.d);
    f4.d = static_cast<double>(PpcForceSingleValueInline(f5.d + f4.d));
    f4.d = static_cast<double>(PpcForceSingleValueInline(f6.d + f4.d));
    f4.d = (-(f4.d));
    f4.d = PpcFmulsInline(f3.d, f4.d);
    MemoryInline::WriteResolvedFloat32(guest_range_2, 12u, (r31 + 12), f4.d);
    f4.d = MemoryInline::FlatReadFloat32((r1 + 20));
    f4.d = PpcFmulsInline(f4.d, f3.d);
    MemoryInline::WriteResolvedFloat32(guest_range_2, 16u, (r31 + 16), f4.d);
    f4.d = MemoryInline::FlatReadFloat32((r1 + 24));
    f4.d = PpcFmulsInline(f4.d, f3.d);
    MemoryInline::WriteResolvedFloat32(guest_range_2, 20u, (r31 + 20), f4.d);
    f4.d = MemoryInline::FlatReadFloat32((r1 + 28));
    f4.d = PpcFmulsInline(f4.d, f3.d);
    MemoryInline::WriteResolvedFloat32(guest_range_2, 24u, (r31 + 24), f4.d);
    f5.d = MemoryInline::FlatReadFloat32((r1 + 20));
    f4.d = MemoryInline::FlatReadFloat32((r1 + 24));
    f6.d = MemoryInline::FlatReadFloat32((r1 + 28));
    f5.d = PpcFmulsInline(f1.d, f5.d);
    f4.d = PpcFmulsInline(f0.d, f4.d);
    f6.d = PpcFmulsInline(f2.d, f6.d);
    f4.d = static_cast<double>(PpcForceSingleValueInline(f5.d + f4.d));
    f4.d = static_cast<double>(PpcForceSingleValueInline(f6.d + f4.d));
    f4.d = (-(f4.d));
    f3.d = PpcFmulsInline(f3.d, f4.d);
    MemoryInline::WriteResolvedFloat32(guest_range_2, 28u, (r31 + 28), f3.d);
    f3.d = MemoryInline::FlatReadFloat32((r1 + 44));
    MemoryInline::WriteResolvedFloat32(guest_range_2, 32u, (r31 + 32), f3.d);
    f3.d = MemoryInline::FlatReadFloat32((r1 + 48));
    MemoryInline::WriteResolvedFloat32(guest_range_2, 36u, (r31 + 36), f3.d);
    f3.d = MemoryInline::FlatReadFloat32((r1 + 52));
    MemoryInline::WriteResolvedFloat32(guest_range_2, 40u, (r31 + 40), f3.d);
    f4.d = MemoryInline::FlatReadFloat32((r1 + 44));
    f3.d = MemoryInline::FlatReadFloat32((r1 + 48));
    f5.d = MemoryInline::FlatReadFloat32((r1 + 52));
    f1.d = PpcFmulsInline(f1.d, f4.d);
    f0.d = PpcFmulsInline(f0.d, f3.d);
    f2.d = PpcFmulsInline(f2.d, f5.d);
    f0.d = static_cast<double>(PpcForceSingleValueInline(f1.d + f0.d));
    f0.d = static_cast<double>(PpcForceSingleValueInline(f2.d + f0.d));
    f0.d = (-(f0.d));
    MemoryInline::WriteResolvedFloat32(guest_range_2, 44u, (r31 + 44), f0.d);
    goto loc_8023F484;
}

loc_8023F47C:
{
    r3 = r31;
    ctx->gpr[1] = r1;
    ctx->gpr[3] = r3;
    ctx->fpr[0] = f0;
    ctx->fpr[1] = f1;
    ctx->fpr[2] = f2;
    InvokeDirectCpu<0x80199D04u>(ctx);
    f0 = ctx->fpr[0];
    f1 = ctx->fpr[1];
    f2 = ctx->fpr[2];
}

loc_8023F484:
{
    // psq_load w=0 quant=0 (using PPC_PsqL)
    r0 = MemoryInline::FlatRead32((r1 + 100));
    f31.d = MemoryInline::FlatReadFloat64((r1 + 80));
    r31 = MemoryInline::FlatRead32((r1 + 76));
    r30 = MemoryInline::FlatRead32((r1 + 72));
    ctx->lr = r0;
    r1 = (r1 + 96);
    ctx->gpr[0] = r0;
    ctx->gpr[1] = r1;
    ctx->gpr[3] = r3;
    ctx->gpr[4] = r4;
    ctx->gpr[5] = r5;
    ctx->gpr[30] = r30;
    ctx->gpr[31] = r31;
    ctx->fpr[0] = f0;
    ctx->fpr[1] = f1;
    ctx->fpr[2] = f2;
    ctx->fpr[3] = f3;
    ctx->fpr[4] = f4;
    ctx->fpr[5] = f5;
    ctx->fpr[6] = f6;
    ctx->fpr[7] = f7;
    ctx->fpr[8] = f8;
    ctx->fpr[9] = f9;
    ctx->fpr[10] = f10;
    ctx->fpr[31] = f31;
    ctx->cr = cr;
    return;
}

}

// RECOMP_GUEST_ABI gpr_read=0xC000000E gpr_write=0xC000003B gpr_return=0x00000018 fpr_read=0x8000072E fpr_write=0x800007FF fpr_return=0x00000002 cr_read=0x01 cr_write=0x01 xer_read=1 xer_write=0 fence=0
// RECOMP_REGISTRATION base 0x8023F230 func_8023F230 preserves=false fpr_mask=0x80000000
