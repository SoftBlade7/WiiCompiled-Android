#include <cstdint>
#include "ppc_runtime.h"
#include "abi_bridge.h"
#include "memory.h"
#include "recomp_mod_loader.h"

extern "C" void func_801DDA08(CpuContext* MKW_RESTRICT ctx)
{
    uint32_t cr0_0 = 0;
    uint32_t r0_rot_0 = 0;

    uint32_t r0 = ctx->gpr[0];
    uint32_t r3 = ctx->gpr[3];
    uint32_t r4 = ctx->gpr[4];
    uint32_t r5 = ctx->gpr[5];
    uint32_t r6 = ctx->gpr[6];
    uint32_t cr = ctx->cr;
    uint32_t xer = ctx->xer;

    goto loc_801DDA08;

loc_801DDA08:
{
    r4 = MemoryInline::FlatRead32((r3 + 4));
    r0 = (r4 & 256);
    SetCRResident(cr, xer, 0, static_cast<int32_t>(r0), static_cast<int32_t>(0));
}

loc_801DDA10:
{
    if (((cr & 0x20000000u) != 0)) {
        goto loc_801DDA1C;
    }
}

loc_801DDA14:
{
    r0 = (r4 & 512);
    SetCRResident(cr, xer, 0, static_cast<int32_t>(r0), static_cast<int32_t>(0));
}

loc_801DDA18:
{
    if (((cr & 0x20000000u) != 0)) {
        goto loc_801DDA24;
    }
}

loc_801DDA1C:
{
    r3 = -7;
    ctx->gpr[0] = r0;
    ctx->gpr[3] = r3;
    ctx->gpr[4] = r4;
    ctx->cr = cr;
    return;
}

loc_801DDA24:
{
    r0 = (r4 & 1);
    SetCRResident(cr, xer, 0, static_cast<int32_t>(r0), static_cast<int32_t>(0));
}

loc_801DDA28:
{
    if (((cr & 0x20000000u) == 0)) {
        goto loc_801DDA34;
    }
}

loc_801DDA2C:
{
    r3 = -4;
    ctx->gpr[0] = r0;
    ctx->gpr[3] = r3;
    ctx->gpr[4] = r4;
    ctx->cr = cr;
    return;
}

loc_801DDA34:
{
    r0 = MemoryInline::FlatRead8((r3 + 200));
    SetCRResident(cr, xer, 0, static_cast<uint32_t>(r0), static_cast<uint32_t>(8));
}

loc_801DDA3C:
{
    if (((cr & 0x80000000u) != 0)) {
        goto loc_801DDA48;
    }
}

loc_801DDA40:
{
    r3 = -6;
    ctx->gpr[0] = r0;
    ctx->gpr[3] = r3;
    ctx->gpr[4] = r4;
    ctx->cr = cr;
    return;
}

loc_801DDA48:
{
    r0_rot_0 = PpcRotl32Inline(static_cast<uint32_t>(r0), static_cast<uint32_t>(3));
    r0 = (r0_rot_0 & 2040);
    r4 = (r3 + r0);
    MemoryInline::FlatWrite32((r4 + 140), r6);
    MemoryInline::FlatWrite32((r4 + 136), r5);
    r4 = MemoryInline::FlatRead8((r3 + 200));
    r0 = (r4 + 1);
    MemoryInline::FlatWrite8((r3 + 200), static_cast<uint8_t>(r0));
    r3 = 0;
    ctx->gpr[0] = r0;
    ctx->gpr[3] = r3;
    ctx->gpr[4] = r4;
    ctx->cr = cr;
    return;
}

}

// RECOMP_GUEST_ABI gpr_read=0x00000079 gpr_write=0x00000019 gpr_return=0x00000018 fpr_read=0x00000000 fpr_write=0x00000000 fpr_return=0x00000000 cr_read=0x01 cr_write=0x01 xer_read=1 xer_write=0 fence=0
// RECOMP_REGISTRATION base 0x801DDA08 func_801DDA08 preserves=true fpr_mask=0x00000000
