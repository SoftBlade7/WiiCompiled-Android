#include <cstdint>
#include "ppc_runtime.h"
#include "abi_bridge.h"
#include "memory.h"
#include "recomp_mod_loader.h"

extern "C" void func_80830134(CpuContext* MKW_RESTRICT ctx)
{
    uint32_t cr0_0 = 0;

    uint32_t r0 = ctx->gpr[0];
    uint32_t r3 = ctx->gpr[3];
    uint32_t r4 = ctx->gpr[4];
    uint32_t cr = ctx->cr;
    uint32_t xer = ctx->xer;

    goto loc_80830134;

loc_80830134:
{
    SetCRResident(cr, xer, 0, static_cast<int32_t>(r4), static_cast<int32_t>(18));
}

loc_80830138:
{
    if (((cr & 0x80000000u) == 0)) {
        goto loc_80830144;
    }
}

loc_8083013C:
{
    r3 = 1;
    ctx->gpr[0] = r0;
    ctx->gpr[3] = r3;
    ctx->cr = cr;
    return;
}

loc_80830144:
{
    r0 = (r4 + -34);
    SetCRResident(cr, xer, 0, static_cast<uint32_t>(r0), static_cast<uint32_t>(1));
}

loc_8083014C:
{
    if (((cr & 0x40000000u) == 0)) {
        goto loc_80830170;
    }
}

loc_80830150:
{
    SetCRResident(cr, xer, 0, static_cast<int32_t>(r4), static_cast<int32_t>(22));
}

loc_80830154:
{
    if (((cr & 0x20000000u) != 0)) {
        goto loc_80830170;
    }
}

loc_80830158:
{
    SetCRResident(cr, xer, 0, static_cast<int32_t>(r4), static_cast<int32_t>(31));
}

loc_8083015C:
{
    if (((cr & 0x20000000u) != 0)) {
        goto loc_80830170;
    }
}

loc_80830160:
{
    SetCRResident(cr, xer, 0, static_cast<int32_t>(r4), static_cast<int32_t>(26));
}

loc_80830164:
{
    if (((cr & 0x20000000u) != 0)) {
        goto loc_80830170;
    }
}

loc_80830168:
{
    SetCRResident(cr, xer, 0, static_cast<int32_t>(r4), static_cast<int32_t>(29));
}

loc_8083016C:
{
    if (((cr & 0x20000000u) == 0)) {
        goto loc_80830178;
    }
}

loc_80830170:
{
    r3 = 3;
    ctx->gpr[0] = r0;
    ctx->gpr[3] = r3;
    ctx->cr = cr;
    return;
}

loc_80830178:
{
    r3 = 2;
    ctx->gpr[0] = r0;
    ctx->gpr[3] = r3;
    ctx->cr = cr;
    return;
}

}

// RECOMP_GUEST_ABI gpr_read=0x00000010 gpr_write=0x00000009 gpr_return=0x00000008 fpr_read=0x00000000 fpr_write=0x00000000 fpr_return=0x00000000 cr_read=0x01 cr_write=0x01 xer_read=1 xer_write=0 fence=0
// RECOMP_REGISTRATION base 0x80830134 func_80830134 preserves=true fpr_mask=0x00000000
