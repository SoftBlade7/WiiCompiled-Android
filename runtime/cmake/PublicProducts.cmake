...
function(mkw_configure_product target)
    ...
    if(NOT CMAKE_SYSTEM_NAME STREQUAL "Android")
        ...
        # SDL_main is never called from any other compiled code - Android's Java-side
        # SDLActivity finds it purely via dlsym() at runtime (see
        # org/libsdl/app/SDLActivity.java). With nothing in the visible call graph
        # referencing it, the linker's unused-symbol elimination silently drops it even
        # though the SDL_MAIN_HANDLED guard above correctly produces a function actually
        # named SDL_main. -u forces the linker to treat it as referenced; --export-dynamic
        # keeps it in .dynsym, which is the table dlsym() searches (a stripped .symtab
        # alone is not enough - see CXX1429/"Couldn't find function SDL_main").
        target_link_options(${target} PRIVATE "-Wl,--export-dynamic,-u,SDL_main")
    endif()
