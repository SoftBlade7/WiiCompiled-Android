...
bool initialize(AuroraBackend auroraBackend) {
  ...
  uint32_t maxTextureDimension2D = 0;
  {
    ...
    // Dawn version (its generated webgpu_cpp.h no longer treats it as a plain aggregate), so a
    // single brace-init wrapped in parens no longer resolves to any constructor. A double-brace
    // {{...}} was tried next but still failed to bind - list-initialization doesn't implicitly
    // construct a named nested Init struct through an Init&&-taking constructor parameter this
    // way. Constructing DawnCacheDeviceDescriptor::Init explicitly by name and passing it as an
    // rvalue unambiguously selects the DawnCacheDeviceDescriptor(Init&&) constructor confirmed
    // to exist in this version's generated webgpu_cpp.h.
    wgpu::DawnCacheDeviceDescriptor::Init cacheDescriptorInit{
        .isolationKey = nullptr,
        .loadDataFunction = load_from_cache,
        .storeDataFunction = store_to_cache,
        .functionUserdata = nullptr,
    };
    wgpu::DawnCacheDeviceDescriptor cacheDescriptor{std::move(cacheDescriptorInit)};
