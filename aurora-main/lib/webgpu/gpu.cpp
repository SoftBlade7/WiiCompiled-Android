...
bool initialize(AuroraBackend auroraBackend) {
  ...
  uint32_t maxTextureDimension2D = 0;
  {
    ...
    // Dawn version (its generated webgpu_cpp.h no longer treats it as a plain aggregate). Two
    // brace-init approaches were tried and both failed to compile against this version's actual
    // generated header: a single brace wrapped in parens (no matching constructor), then a
    // double-brace via an explicitly-named ::Init struct (Init has no such field names in this
    // version - "field designator does not refer to any field"). Default-construct and assign
    // members directly instead, matching the pattern Dawn's own current test suite uses for this
    // exact type (dawn.googlesource.com/dawn cacheDesc = {}; cacheDesc.isolationKey = ...) - this
    // only depends on the plain public member names, not on any particular constructor shape, so
    // it should be stable across Dawn versions going forward.
    wgpu::DawnCacheDeviceDescriptor cacheDescriptor{};
    cacheDescriptor.isolationKey = nullptr;
