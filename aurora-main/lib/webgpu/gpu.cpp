...
bool initialize(AuroraBackend auroraBackend) {
  ...
  uint32_t maxTextureDimension2D = 0;
  {
    ...
    // Dawn version (its generated webgpu_cpp.h no longer treats it as a plain aggregate). Three
    ...
    // explicitly-named ::Init struct (Init has no such field names in this version), and direct
    // member assignment after default-construction (no such members on the struct either). A CI
    // step was added to dump the real generated header for this exact build, which showed this
    // Dawn version has moved off raw function-pointer fields entirely: DawnCacheDeviceDescriptor
    // now only has isolationKey (a StringView, default-initialized to {}, so nothing to set here
    // since we don't use a cache isolation key) plus two CallbackInfo members that are set via
    // templated SetDawnLoadCacheDataCallback(F)/SetDawnStoreCacheDataCallback(F) methods rather
    // than plain field assignment - F is deduced from whatever's passed, and Dawn's own
    // CallbackInfoHelper wraps a plain C function pointer of the expected signature into the
    // callback-info struct internally, so load_from_cache/store_to_cache (declared in gpu.hpp
    // with the classic WGPUDawnLoadCacheDataFunction/WGPUDawnStoreCacheDataFunction C signatures)
    // can be passed directly without any wrapping on our side.
