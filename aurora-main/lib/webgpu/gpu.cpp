...
bool initialize(AuroraBackend auroraBackend) {
  ...
  uint32_t maxTextureDimension2D = 0;
  {
...
#ifdef WEBGPU_DAWN
    // DawnCacheDeviceDescriptor inherits from ChainedStruct and a protected detail base in this
    // Dawn version (its generated webgpu_cpp.h no longer treats it as a plain aggregate). Four
    // approaches were tried and all failed to compile against this version's actual generated
    // header: a single brace wrapped in parens (no matching constructor), a double-brace via an
    // explicitly-named ::Init struct (Init has no such field names), direct member assignment
    // after default-construction (no such members on the struct either), and passing
    // load_from_cache/store_to_cache directly to SetDawnLoadCacheDataCallback/
    // SetDawnStoreCacheDataCallback (compiled up through template instantiation, then failed:
    // "implicit instantiation of undefined template CArgConverter<...>"). The CI diagnostic dump
    // showed why: CArgConverter's only specializations for these two CallbackInfo types expect a
    // span-based C++ signature - size_t(std::span<const std::byte> key, std::span<std::byte>
    // value) for load, void(std::span<const std::byte> key, std::span<const std::byte> value)
    // for store - not the classic C signature (void const* key, size_t keySize, void* value,
    // size_t valueSize, void* userdata) that load_from_cache/store_to_cache actually have; there
    // is no userdata parameter in the C++-facing signature at all, since CallbackInfoHelper
    // threads that through separately via userdata1/userdata2 on the CInfoT struct itself.
    // Non-capturing lambdas bridge the two: they match the span-based shape the specialization
    // wants, and translate straight back to the existing, already-correct pointer+size
    // implementations. Must stay non-capturing (no [] captures) so CallbackInfoHelper's
    // non-capturing branch applies (decays to a plain function pointer via unary +, matching
    // CallbackHelper's required R(*)(CArgs...) shape) rather than its capturing-lambda branch,
    // which requires an explicit CallbackMode and isn't what a plain DawnCacheDeviceDescriptor
    // wants here. load_from_cache's probe-then-fetch protocol (a null `value` pointer means
    // "just return the size, don't fill anything in yet") is preserved: an empty/default value
    // span has value.data() == nullptr, which is exactly the sentinel the existing
    // implementation already checks for.
    ...
    cacheDescriptor.SetDawnStoreCacheDataCallback(
        [](std::span<const std::byte> key, std::span<const std::byte> value) -> void {
